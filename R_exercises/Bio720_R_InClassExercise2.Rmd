...
So in the first function you are calling x as a global variable. While in the second function x is only called in the function itself so when the function is done running x is killed as a variable and no longer exists.
```{r}
x <- 1
...
If you can it is best to set all of your variables at the same time
